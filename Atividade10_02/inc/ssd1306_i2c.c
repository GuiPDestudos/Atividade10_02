...
void ssd1306_clear_display(ssd1306_t *ssd) {
    memset(ssd->ram_buffer + 1, 0x00, ssd->bufsize - 1);  // Preenche o buffer com 0
    ssd1306_send_data(ssd);  // Envia o buffer ao display
}

void ssd1306_draw_filled_rectangle(uint8_t *ssd, int x, int y, int width, int height) {
    for (int i = x; i < x + width; i++) {
        for (int j = y; j < y + height; j++) {
            ssd1306_set_pixel(ssd, i, j, true);  // Define o pixel como aceso
        }
    }
}
